...
export const decodeToken = (req, res, next) => {
    ...
    if (!tokenHeader) return res.status(401).json({message: "Token não informado"});
...
export const adminOnly = (req, res, next) => {
  ...
  if (req.user.role !== "admin") return res.status(403).json({message: "Você não possui permissões para executar esta ação"});
  next();
};
...
export const validateIsTheAdminOrUser = (req, res, next) => {
  ...
  return res.status(403).json({message: "Você não possui permissões para executar esta ação"})
};
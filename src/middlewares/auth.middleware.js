...
export const decodeToken = (req, res, next) => {
    ...
    if (!tokenHeader) return res.status(400).json({message: "Token não informado"});
...
export const adminOnly = (req, res, next) => {
  ...
  if (req.user.role !== "admin") return res.status(401).json({message: "Acesso negado, somente administradores podem realizar esta ação"});
  next();
};
...
export const validateIsTheAdminOrUser = (req, res, next) => {
  ...
  return res.status(401).json({message: "Acesso negado - Você não possui permissão para executar esta ação"})
};